...
#' @param colNames Name of each variable in the output.
#' @param sigmaPerSeason whether each site/season has a separate process error
...
#' colMeans(summ$sigma)
#' 
getGibbsSummary <- function(ssOut, burnin = NULL , colNames = NULL, sigmaPerSeason=T){
  p <- ncol(ssOut$data$x)
